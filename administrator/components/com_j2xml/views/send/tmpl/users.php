...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_j2xml'); ?>"
	...
	<?php echo \Joomla\CMS\HTML\HTMLHelper::_($ui . '.startTabSet', 'j2xmlUsers', array('active' => 'export')); ?>

	<?php foreach ($fieldsets as $name => $fieldSet) : ?>
		<?php if ($name == 'details') continue; ?>

		<?php $label = empty($fieldSet->label) ? 'COM_J2XML_' . $name . '_FIELDSET_LABEL' : $fieldSet->label; ?>
		<?php echo \Joomla\CMS\HTML\HTMLHelper::_($ui . '.addTab', 'j2xmlUsers', $name, Text::_($label)); ?>
	...
	<button class="hidden" id="j2xmlSendOkBtn" type="button"
		onclick="
			...
			if (document.formvalidator.isValid(f)) {
				...
				eshiol.j2xml.send({
					export_url: 'index.php?option=com_j2xml&task=users.export&format=json&<?php echo \Joomla\CMS\Session\Session::getFormToken(); ?>=1',
