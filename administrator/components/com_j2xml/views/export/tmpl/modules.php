...
<form
	action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_j2xml&task=modules.display&format=raw'); ?>"
	id="adminForm" method="post" name="adminForm" autocomplete="off"
	class="form-horizontal">

	<?php $fieldsets = $this->form->getFieldsets(); ?>

	<?php echo \Joomla\CMS\HTML\HTMLHelper::_($ui . '.startTabSet', 'j2xmlModules', array('active' => 'export')); ?>

	<?php foreach ($fieldsets as $name => $fieldSet) : ?>
		<?php if ($name == 'details') continue; ?>

		<?php $label = empty($fieldSet->label) ? 'COM_J2XML_' . $name . '_FIELDSET_LABEL' : $fieldSet->label; ?>
		<?php echo \Joomla\CMS\HTML\HTMLHelper::_($ui . '.addTab', 'j2xmlModules', $name, Text::_($label)); ?>
