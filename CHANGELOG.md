...
### Changed
- **Eliminated duplicate code in Content.php and User.php** — extracted the custom field/subform alias-building logic (50 identical lines) into a shared `buildFieldAliases()` method in the base `Table` class. Also extracted the subform field export logic (30 identical lines) into a shared `exportFields()` static method. Resolves CodeFactor duplicate-code findings.
