...
class Fieldgroup extends Table
{
    ...
    public static function import ($xml, &$params, $db = null, $userId = null)
    {
        ...
        foreach ($xml->xpath("//j2xml/fieldgroup") as $record)
        {
            ...
            if (!$fieldgroup )
            {
                ...
                $table->bind($data);
                self::storeImportedTable($table, 'LIB_J2XML_MSG_FIELDGROUP_IMPORTED', 'LIB_J2XML_MSG_FIELDGROUP_NOT_IMPORTED', $data['title'], $table->title);
    ...
    public static function export ($id, &$xml, $options, $db = null)
    {
        ...
        $item = static::loadExportItem('fieldgroup', $id, $xml, $db);
        if (!$item)
        {
            return;
        }
